...
def node_generate_limitations(state: SectionBasedState) -> Dict[str, Any]:
    """Node 6 : Generate Limitations section using selected prompt."""
    start_time = time.time()
    prompt_limitations = state["prompt_limitations"]
    print(f"[STEP] Generating Limitations with prompt {prompt_limitations['id']}")
    ...
    user_prompt = prompt_limitations["user"].format(
        abstracts_text=abstracts_text,
        introduction=introduction,
        analysis=analysis,
        discussion=discussion
    )
    system_message = prompt_limitations.get("system", "You are a helpful research assistant.")
    ...
    print(f"  → Prompt: {prompt_limitations['id']} (Style: {prompt_limitations['style']})")
    print(f"  → Generated {len(response.content)} chars")
    end_time = time.time()
    execution_time = end_time - start_time
    ...
    state.update({
        "messages": [AIMessage(content=f"[{prompt_limitations['id']}] Limitations generated")],
...
def build_workflow() -> StateGraph:
    ...
    workflow.add_node("generate_limitations", node_generate_limitations)
    ...
    workflow.add_edge("generate_discussion", "generate_limitations")
    workflow.add_edge("generate_limitations", "concatenate")
...
class TimeTravelSummarizer:
    ...
    def __init__(self):
        try:
            db_conn = sqlite3.connect(project_root / 'tmp' / 'langgraph_checkpoints_exp2.sqlite', check_same_thread=False)
        except sqlite3.OperationalError as e:
            print(f"[ERROR] Could not connect to database: {e}")
            # try to create the db file
            os.makedirs(project_root / 'tmp', exist_ok=True)
            with open(project_root / 'tmp' / 'langgraph_checkpoints_exp2.sqlite', 'w') as f:
                pass
            db_conn = sqlite3.connect(project_root / 'tmp' / 'langgraph_checkpoints_exp2.sqlite', check_same_thread=False)
        self.checkpointer = SqliteSaver(db_conn)
    ...
    def run_workflow(self, topic: str, run_id: Optional[str] = None):
        ...
        for i, snapshot in enumerate(state_history):
            print(f"  Checkpoint {i}: next={snapshot.next}, checkpoint_id={snapshot.config['configurable'].get('checkpoint_id', 'N/A')}...")
        ...
        for intro_idx, intro_prompt in enumerate(prompts_intro):
            ...
            # intro_step_config["configurable"]["thread_id"] = f"branch_I{intro_idx}"   
            ...
            for analysis_idx, analysis_prompt in enumerate(prompts_analysis):
                ...
                for discussion_idx, discussion_prompt in enumerate(prompts_discussion):
                    ...
                    nodes_before_limitations = nodes_before_analysis.copy()
                    for event in self.app.stream(None, discussion_step_config, stream_mode="updates"):
                        for node_name, node_output in event.items():
                            nodes_before_limitations.append(node_name)
                            print(f"  [OK] {node_name}")
                        ...
                        if "generate_discussion" in nodes_before_limitations:
                            break

                    state_history = list(self.app.get_state_history(config))
                    discussion_checkpoint = None
                    for state_snapshot in state_history:
                        if state_snapshot.next == ("generate_limitations",):
                            discussion_checkpoint = state_snapshot
                            break
                        
                    for limitations_idx, limitations_prompt in enumerate(prompts_limitations):
                        
                        # Run discussion step to get to next checkpoint
                        limitations_step_config = discussion_checkpoint.config
                        limitations_step_config = self.app.update_state(
                            limitations_step_config,
                            values={"prompt_limitations": limitations_prompt}
                        )
                        # Finally, run discussion → concatenate
                        final_state = None
                        nodes_all = nodes_before_limitations.copy()
                        for event in self.app.stream(None, limitations_step_config, stream_mode="updates"):
                            for node_name, node_output in event.items():
                                nodes_all.append(node_name)
                                print(f"  [OK] {node_name}")
                                if node_name == "generate_limitations":
                                    token_usage_overall["prompt_tokens"] += node_output.get("experiment_result", {}).get("limitations_tokens", {}).get("prompt_tokens", 0)
                                    token_usage_overall["completion_tokens"] += node_output.get("experiment_result", {}).get("limitations_tokens", {}).get("completion_tokens", 0)
                                    token_usage_overall["total_tokens"] += node_output.get("experiment_result", {}).get("limitations_tokens", {}).get("total_tokens", 0)

                                # Verify we're NOT re-running search or extract
                                if node_name in ["search_papers", "extract_abstracts", "generate_introduction", "generate_analysis", "generate_discussion"]:
                        ...
                        if final_state:
                            ...
                            result = {
                                ...
                                "prompt_limitations": final_state["prompt_limitations"]["id"],
                                "prompt_limitations_style": final_state["prompt_limitations"]["style"],
                                ...
                                "metadata": {
                                    ...
                                    "breakdown": {
                                        ...
                                        "limitations_tokens": final_state["experiment_result"]["limitations_tokens"],
                                    }
                                }
                            }
